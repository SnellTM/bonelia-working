  ...
  return (
    <div className={`cakeWrap ${type}`}>
      <div className="cake">
        ...
        {type === 'chocolate' && <div className="chips">{Array.from({length: 9}).map((_,i)=><i key={i}></i>)}</div>}
    ...
    <main>
        ...
        <div className="flavorGrid">
          {flavors.map((flavor, i) => (
            <button
              ...
              className={`flavorCard reveal ${selected.id === flavor.id ? 'active' : ''}`}
      ...
      <section id="pedidos" className="section orderSection">
        <div className="sectionTitle reveal">
          ...
          <p>La página está pensada para que el cliente te escriba sin vueltas.</p>
        </div>
        ...
        <div className="steps">
          {[
            ...
            <div className="step reveal" style={{animationDelay: `${i * .08}s`}} key={title}>
              <Icon/>
              <h3>{title}</h3>
              <p>{text}</p>
            </div>
