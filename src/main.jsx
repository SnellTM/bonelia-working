...
/*
  CÓMO CAMBIAR FOTOS DESPUÉS:
  1) Subí la foto a src/assets/ con un nombre simple: limon.png, chocolate.png, etc.
  2) Agregá arriba: import limonFoto from "./assets/limon.png";
  3) En el sabor poné: foto: limonFoto,
*/
...
const flavors = [
  ...
  },
  {
    ...
    descripcion: "Budín marmolado de vainilla y chocolate, decorado con hilos artesanales de ganache de chocolate oscuro y chocolate blanco.",
    precio: "$ 5.000",
    clase: "marmolado",
    foto: marmoladoFoto,
...
function Logo({ compact = false, word = true }) {
  return (
    <div className={`realLogo ${compact ? "realLogoCompact" : ""}`}>
      ...
      {word && <img className="realLogoWord" src={logoWordmark} alt="Bonelia Pastelería Boutique" />}
    </div>
  );
}
  ...
  return (
    <div className={`cakeScene ${type}`}>
      ...
      <div className="cakeLoaf">
        ...
        {type === "limon" && <><div className="zest">{Array.from({ length: 12 }).map((_, i) => <b key={i}></b>)}</div><div className="lemonSlices"><span></span><span></span><span></span></div></>}
        {type === "chocolate" && <div className="chips">{Array.from({ length: 9 }).map((_, i) => <b key={i}></b>)}</div>}
        {type === "frutos" && <div className="nuts">{Array.from({ length: 10 }).map((_, i) => <b key={i}></b>)}</div>}
        {type === "vainilla" && <div className="zest soft">{Array.from({ length: 10 }).map((_, i) => <b key={i}></b>)}</div>}
      </div>
    </div>
  );
}
    ...
    <main>
      ...
      <section id="inicio" className="hero section">
        ...
        <div className="heroText reveal">
          ...
          <div className="heroLogo"><Logo word={true} /></div>
        ...
        <div className="heroProduct reveal delay"><FlavorVisual flavor={flavors[4]} featured /></div>
      ...
      <section id="sabores" className="section flavorsSection">
        ...
        <div className="flavorGrid">
          {flavors.map((flavor, i) => (
            <button key={flavor.id} onClick={() => setSelected(flavor)} className={`flavorCard reveal ${selected.id === flavor.id ? "active" : ""} ${flavor.foto ? "withPhoto" : ""}`} style={{ animationDelay: `${i * 0.08}s` }}>
              <FlavorVisual flavor={flavor} />
              <h3>{flavor.carta}</h3><p>{flavor.texto}</p><strong>{flavor.precio}</strong>
            </button>
          ))}
        </div>
        <div className="selectedPanel reveal">
          <div className="selectedVisual"><FlavorVisual flavor={selected} featured /></div>
          <div className="selectedText"><small>Seleccionado</small><h2>{selected.carta}</h2><h3>{selected.nombre}</h3><p>{selected.descripcion}</p><div className="tags">{selected.items.map((item) => <span key={item}>{item}</span>)}</div><div className="priceRow"><strong>{selected.precio}</strong><a className="primary" href={whatsappLink(selected.carta)} target="_blank" rel="noreferrer"><MessageCircle size={19} /> Pedir este sabor</a></div></div>
        </div>
      </section>
      ...
      <section id="pedidos" className="section orderSection">
        ...
        <div className="sectionTitle reveal"><small>PEDIDOS</small><h2>Simple, rápido y con cupos limitados</h2></div>
        <div className="steps">{[[Heart,"Elegí","Mirá los sabores disponibles."],[MessageCircle,"Escribinos","El botón abre WhatsApp con el pedido armado."],[Truck,"Coordinamos","Retiro en Parodi 236 o envío a coordinar."],[Gift,"Disfrutá","Tu budín sale cuidado y fresco."]].map(([Icon, title, text], i) => <div className="step reveal" style={{ animationDelay: `${i * 0.08}s` }} key={title}><em>{i + 1}</em><Icon /><h3>{title}</h3><p>{text}</p></div>)}</div>
      </section>

      <section className="section ctaSection reveal"><div className="ctaCard"><Logo word={false} /><div><h2>Pedidos abiertos para el fin de semana</h2><p>Tomamos pedidos jueves y viernes. Los cupos son limitados para mantener la calidad.</p></div><div className="actions ctaActions"><a className="lightBtn" href={whatsappLink()} target="_blank" rel="noreferrer"><MessageCircle size={18} /> WhatsApp principal</a><a className="lightBtn" href={whatsappLink("", WHATSAPP_NUMBERS.alternativo)} target="_blank" rel="noreferrer"><MessageCircle size={18} /> WhatsApp alternativo</a></div></div></section>
