...
import { Heart, MessageCircle, Truck, Clock, MapPin, Camera, Sparkles, PackageCheck, Star, Menu, X, Scale, ChefHat, Gift } from "lucide-react";
    ...
    <main>
      ...
      <section id="sabores" className="section flavorsSection">
        ...
        <div className="flavorGrid">
          {flavors.map((flavor, i) => <button key={flavor.id} onClick={() => setSelected(flavor)} className={`flavorCard reveal ${selected.id === flavor.id ? 'active' : ''}`} style={{animationDelay: `${i * .08}s`}}><Cake type={flavor.clase}/><h3>{flavor.carta}</h3><p>{flavor.texto}</p><strong>{flavor.precio}</strong></button>)}
        </div>
